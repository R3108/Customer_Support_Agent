...
export function DemoPlayground() {
  ...
  const resetOrders = async () => {
    try {
      ...
      customers.reload();
      setResetNote("Demo orders restored");
    } catch (e) {
      setResetNote(e instanceof Error ? e.message : "Reset failed");
    }
    window.setTimeout(() => setResetNote(null), 3000);
  };
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[280px_minmax(0,1fr)_360px]">
      ...
      <aside className="hidden min-h-0 flex-col overflow-y-auto border-r border-slate-200 bg-white lg:flex">
              ...
              <div className="mt-1.5 flex flex-wrap gap-1">
                {active.orders.map((o) => (
                  <button
                    ...
                    className="rounded border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[10px] text-slate-700 hover:border-brand-300 hover:text-brand-700"
        ...
        <section className="p-4">
            ...
            {SCENARIOS.map((s) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="shrink-0 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-600">{s.tag}</span>
                </div>
          ...
          <button
            ...
            className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg px-2 py-1.5 text-xs text-slate-500 hover:bg-slate-50 hover:text-slate-800"
            title="Undo cancellations, returns and refunds made during the demo"
          >
            <RotateCcw className="h-3.5 w-3.5" /> {resetNote ?? "Reset demo orders"}
          </button>
        </section>
      </aside>
