...
export function ChatPanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-white">
      ...
      <div ref={scrollRef} className={cx("flex-1 space-y-4 overflow-y-auto bg-slate-50/60", compact ? "px-3 py-4" : "px-5 py-5")}>
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-2">
                {suggestions.map((s) => (
                  <button
                    key={s}
                    onClick={() => submit(s)}
                    className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-700 shadow-sm transition hover:border-brand-300 hover:text-brand-700"
                  >
                    {s}
                  </button>
                ))}
        ...
        {openOffer && (
          <div className="animate-bubble flex flex-wrap gap-2 pl-9">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3.5 py-1.5 text-xs text-slate-700 transition hover:bg-slate-50"
            >
        ...
        {showRating && (
          <div className="animate-bubble flex flex-col items-center gap-1.5 pt-2">
            <span className="text-xs text-slate-500">How&apos;s your support experience so far?</span>
      ...
      {chat.error && (
        <div className="flex items-center gap-2 border-t border-rose-200 bg-rose-50 px-4 py-2 text-xs text-rose-700">
          <ShieldAlert className="h-4 w-4 shrink-0" /> {chat.error}
        </div>
      )}

      <form
        className={cx("border-t border-slate-200 bg-white", compact ? "p-3" : "p-4")}
        onSubmit={(e) => {
          e.preventDefault();
          void submit(draft);
        }}
      >
        <div className="flex items-end gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm focus-within:border-brand-400 focus-within:ring-2 focus-within:ring-brand-100">
          <textarea
            ref={inputRef}
            value={draft}
            ...
            className="max-h-32 min-h-[24px] flex-1 resize-none bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400"
          />
          <button
            type="submit"
            disabled={!draft.trim() || chat.pending}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-600 text-white transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
            aria-label="Send message"
          >
            <ArrowUp className="h-4 w-4" />
          </button>
        </div>
        <p className="mt-1.5 text-center text-[10px] text-slate-400">AI replies can make mistakes · type “talk to a human” anytime</p>
      </form>
    </div>
  );
}
...
export function ActionChip({ action }: { action: ActionSummary }) {
  ...
  return <span className="rounded bg-slate-100 px-1.5 py-0.5 text-slate-600">{action.label} · {action.status.replace("_", " ")}</span>;
}
      ...
      <div className={cx("min-w-0", compact ? "max-w-[88%]" : "max-w-[80%]")}>
        ...
        <div
          className={cx(
            ...
            human ? "border-emerald-200 bg-emerald-50" : meta.escalated ? "border-amber-200 bg-white" : "border-slate-200 bg-white",
          )}
        >
          <Markdown>{m.content}</Markdown>
        </div>
        <div className="mt-1 flex flex-wrap items-center gap-1.5 pl-1 text-[11px] text-slate-400">
          ...
          {sources.length > 0 && (
            <button onClick={() => setOpenSources((v) => !v)} className="inline-flex items-center gap-1 rounded px-1 hover:bg-slate-100 hover:text-slate-600">
        ...
        {openSources && (
          <ul className="mt-1.5 space-y-1 pl-1">
            {sources.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px]">
                <span className="truncate text-slate-600">
                  {s.title} › <span className="text-slate-900">{s.section}</span>
                </span>
                {showMeta && <span className="shrink-0 font-mono text-slate-400">{s.score.toFixed(2)}</span>}
              </li>
            ))}
          </ul>
        )}
